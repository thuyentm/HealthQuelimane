...
function getPrescriptionItemsForDispense($prsid,$stock=''){
	...
	SELECT prescribe_items.PRS_ITEM_ID,
        ...
        where prescribe_items.Active = 1 AND prescribe_items.DRGID = drugs.DRGID AND prescribe_items.PRES_ID = '".$prsid."' ";
	$result = mysql_query($sql);
	$count=mysql_num_rows($result);
    if (!$result) return null;
	$out = "";
...
function renderPrescriptionForm($frm,$fName,$type,$opd,$mode){
        ...
        $out .=" }\n";
        $out .=" function drugSelect(drid,dosage,frequency,howlong) {\n";
