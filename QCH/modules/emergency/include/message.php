...
function renderFormEdit($msgid){
		...
		$mdsDB1 = MDSDataBase::GetInstance();
		$mdsDB1->connect();		
		$sql1 = "SELECT user.FirstName,user.OtherName,messages.from_user,messages.to_user,messages.subject,messages.message FROM messages LEFT JOIN user ON user.UID=messages.from_user WHERE message_id = '".$msgid."'";
		$result1=$mdsDB1->mysqlQuery($sql1);
