...
class MDSEmr extends MDSPersistent {
    ...
    public function openId($pid) {
        ...
        if ($this->isOpened)
            $this->css_Cont_class = "opdCont";
        else
            $this->css_Cont_class = " opdContClose";
    }
    ...
    public function loadEmr() {
        ...
        $alert = $this->getValue("Alert");
        if ($alert == "0")
            $alert = "No";if ($alert == "1")
            $alert = "Yes";
        $voice = $this->getValue("Voice");
        if ($voice == "0")
            $voice = "No";if ($voice == "1")
            $voice = "Yes";
        $pain = $this->getValue("Pain");
        if ($pain == "0")
            $pain = "No";if ($pain == "1")
            $pain = "Yes";
        $unr = $this->getValue("UNR");
        if ($unr == "0")
            $unr = "No";if ($unr == "1")
            $unr = "Yes";
        ...
        $out .= "<tr>\n";
        $out .= "<td>Triage Category : " . ($this->getValue("Status") == 'Critical' ? "<font color='red'>" . $this->getValue("Status") . "</font>" : $this->getValue("Status")) . "</td>\n";
        $out .= "</tr>\n";
        $out .= "<tr>\n";
        $out .= "<td style='height:25px'>" . getPrompt("Weight") . " : " . $this->getValue("Weight") . "</td><td>Height : " . $this->getValue("Height") . "</td><td>Sys_BP : " . $this->getValue("sys_BP") . "</td><td>Diast_BP : " . $this->getValue("diast_BP") . "</td><td>Temprature : " . $this->getValue("Temprature") . "</td>"."</td><td>Pulse : " . $this->getValue("Pulse")."<td align='right'>" . $tools . "</td>\n";
        $out .= "</tr>\n";
        $out .= "<tr>\n";
    ...
    public function loadPrescriptionItems() {
        ...
        if ($prscription->getValue("Active") == false)
            return "";
        $mdsDB = MDSDataBase::GetInstance();
        $mdsDB->connect();
        $sql = "
    ...
    public function getQuestionnaires() {
        ...
        $result = $mdsDB->mysqlQuery($sql);
        if (!$result) {
            echo "ERROR getting questionnaire Items";
            return null;
        }
        $count = $mdsDB->getRowsNum($result);
        if ($count == 0)
