...
class MDSPrescription extends MDSPersistent
{
		...
		while($row = mysql_fetch_array($result))  {
			...
			$out .= "<td>".$row["HowLong"]."</td>\n";
                            if ($type == "new" ){
                                ...
                                $out .= "<td nowrap><span class='continue'  onmouseover=\"drugSelect('".$row["DRGID"]."',String('".$row["Dosage"]."'),'".$row["Frequency"]."','".$row["HowLong"]."')\"   onclick=\"saveData()\" >Continue this</span></td>";                            
	...
	public function renderForm($opd,$pat,$mode){
		...
		$out .= "</table>\n";
		$out .= "<table border=0 width=80% class='PrescriptionInfo' style='background:#FFFFFF'>\n";
		$out .= "<tr>";
		$out .= "<td class='pTd'>#</td><td class='pTd'>".getPrompt("Drug")."</td><td nowrap class='pTd'>".getPrompt("Dosage")."</td><td nowrap class='pTd'>".getPrompt("Frequency")."</td><td nowrap class='pTd'>".getPrompt("Period")."</td><td  class='pTd'></td>"; 
		$out .= "</tr>";
	...
	private function getDrugs($c_stock){
		...
		$mdsDB->connect();
                ...
                if ($c_stock == "ClinicStock"){
                    $sql.=" SELECT drugs.DRGID,drugs.Name,drugs.ClinicStock as Stock,drugs.dDosage,drugs.dFrequency FROM drugs,Doctor_Drug where (Doctor_Drug.DRGID = drugs.DRGID ) AND (Doctor_Drug.USRID = '$UID') AND (drugs.Active = 1) ";
		...
		if (!$result) return null;
		$out = "";
		$out .= "<select class='input' id='DRGID' size='10' style='width:300;' autofocus  onchange=setDefault('DRGID') ".$this->isOpened." >\n";
