...
// Document constants
$dat = date('d/m/Y');
$form_title = 'Hospital Staff List ' . ' ' . $dat;
...
$file_name_d = 'staff_list.txt';  //this file also to be passed (10 columns, "|" as delimiter)
...
function Table($header,$data) {
    //Column widths
    $w=array(15,35,20,25,20,18,18,18,10,10);
    //Header
    ...
    foreach($data as $row)
    {
        ...
        $this->Cell($w[2],4,$row[2],0);
        $this->Cell($w[3],4,$row[3],0);
        $this->Cell($w[4],4,$row[4],0);
        $this->Cell($w[5],4,$row[5],0);
        $this->Cell($w[6],4,$row[6],0);
        $this->Cell($w[7],4,$row[7],0);
        $this->Cell($w[8],4,$row[8],0,'','R');
        $this->Cell($w[9],4,$row[9],0,'','R');
        $this->Ln();
    }
    //Closure line
...
//Column titles
$header=array('Title','Name','Login','Post','User group','Appt. date', 'Disch. date','Incr.', 'Cas.', 'Vac.',);
