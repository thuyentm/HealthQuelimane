...
if ($patient) {
    ...
    $pat_hos_d = "Quelimane Central Hospital"; //returns the default hospital
...
//        $pat_nam_d=strlen($pat_nam_d)>25?substr($pat_nam_d,0,25).'...':$pat_nam_d;
        ...
        $this->setBarcode($barcode,$x1+10,$y1+4*$dy);
        $this->SetXY($x1, $y1 + 5.5*$dy);
        $this->SetFont('arial', 'B', 8);
        $this->write(8, $bring);
        $this->SetXY($x1, $y1 + 6*$dy);
        
        /*$this->SetFont('akandynew', '', 8);
        $encode_data = iconv('UTF-8', 'windows-1252', '@r~hlt p#É@zn s$m ìtm @mm k`Dpw ß@gn eNn.');
        $this->write(8, $encode_data);*/
