...
function renderLabOrderForm($frm,$fName,$type,$episode){
	...
	$data  = ' data:({';
	$data .= ' Dept:"'.$type.'", OPDID:"'.$episode->getId().'", PID:"'.$pat->getId().'", OrderDate:"'.date("Y-m-d H:i:s").'", OrderBy:"'.getLoggedUser().'", Status:"Pending", CreateDate:"'.date("Y-m-d H:i:s").'", Active:"1", CreateUser:"'.getLoggedUser().'", Priority:$("#Priority").val(),TestGroupName:$("#labTest").val(),';   
...
function renderLabOrderFormEdit($frm,$fName,$type,$opd,$orderid){
	...
	$data .= ' }),';
         $out .=" $('#saveBtn').attr('disabled','true'); \n";
	$out .=" var resM=$.ajax({ \n";
							   $out .= "url: 'include/admission_lab_order_save.php',\n";
							   $out .= "global: false,\n";
							   $out .= "type: 'POST',\n";
							   $out .= $data."\n";
							   $out .= "async:false\n";
							...
							$out .= " eval(resM); \n";
							//$out .= " if ( !Error) { \n";
							$out .= " self.document.location='home.php?page=laboratory' ; \n ";
							//$out .= "} else { $('#MDSError').html(res); }\n";
	$out .=" }\n"; //function saveData
	$out .=" parent.window.onbeforeunload = confirmExit; \n";
	$out .="</script>\n"; 
