...
//        return array("width" => "75px", "formatter_JS" => MDSPager::YES_NO_FORMATTER_EL,'stype' => 'select', 'editoptions' => array('value' => ":All;1:Yes;0:No"));
    ...
    function encrypt($str, $key=PASSWORD) {
        ...
        $enc = mcrypt_encrypt(MCRYPT_DES, $key, $str, MCRYPT_MODE_ECB);
    ...
    function createColModel() {
        ...
        $mdsDB->connect();
        $result = $mdsDB->mysqlQuery($this->sql_EL);
        ...
        while ($i < mysql_num_fields($result)) {
            ...
            if ($meta) {
                $title = $meta->name;
                $table = $meta->table;
                if(!$table) $column->search=false;
                $column->setIndex($title);
...
//            $this->colModel[ $column->getName()] = $column;
            array_push($this->colIndexes_EL, $column->getIndex());
            $this->colModel_JSAR['"' . $column->getName() . '"'] = $column;
            $i++;
        }
