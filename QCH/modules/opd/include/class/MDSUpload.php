...
class MDSUpload 
{
	...
	private $allowed_types=array("txt","doc","docx","xls","xlsx","rtf","ppt","pdf","jpg","jpeg","gif","png","xml");
...
public function createMsgFolder($from,$to){
		
			$msgFolder ="";
			$msgFolder = $this->getDefaultFolder();
			$this->folder = $this->GetBasePath().$this->path."attach".$this->path.$this->getDefaultFolder();
			if (!file_exists($this->folder)){
				mkdir($this->folder , 0755);
			}			
			$msgFolder = "message";
			$this->folder = $this->GetBasePath().$this->path."attach".$this->path.$this->getDefaultFolder().$this->path.$msgFolder;
			if (!file_exists($this->folder)){
				mkdir($this->folder , 0755);
			}
			$msgFolder = "message".$this->path.$from."_".$to;
			$this->folder = $this->GetBasePath().$this->path."attach".$this->path.$this->getDefaultFolder().$this->path.$msgFolder;
			if (!file_exists($this->folder)){
				mkdir($this->folder , 0755);
			}
