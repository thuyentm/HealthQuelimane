...
class MDSPatient extends MDSPersistent
{
	...
	public function loadHistory($ops = ""){
		...
		$mdsDB = MDSDataBase::GetInstance();
		$mdsDB->connect();		
		$sql=" SELECT * FROM patient_history WHERE PID = '".$this->getId()."' ORDER BY HistoryDate DESC ";
		$result=$mdsDB->mysqlQuery($sql); 
		if (!$result) { echo "ERROR getting Exam Items"; return null; }
		$count = $mdsDB->getRowsNum($result);
		$mdsPermission = MDSPermission::GetInstance();
...
*/
		while($row = $mdsDB->mysqlFetchArray($result))  {
			if ($row["PATHISTORYID"]) {
				$tools ="";
				if ($gotAccess) 
						$tools .= "<img   src='images/edit-icon.png' width=15 height=15 style='cursor:pointer;' title='Edit record' onmousedown=self.document.location='home.php?page=patient_history&PATHISTORYID=".$row["PATHISTORYID"]."&action=Edit&PID=".$this->getId()."'>";
				...
				$out .= "</td>\n";
				$out .= "<td >";
				$out .= $row["History_Type"]; $out .="</td>\n";
				$out .= "<td >";
				$out .= $row["SNOMED_Text"]; $out .="</td>\n";
				$out .= "<td align='right'>By: ".$row["CreateUser"]."</td>\n";
