...
class MDSOpd extends MDSPersistent
{
	public $patient = NULL;
	...
	public function loadOPD($opdid) {
		...
		if ($mdsPermission->haveAccess($_SESSION["UGID"],"opd_visit_Edit"))
				$tools = "<img   src='images/edit-icon.png' width=15 height=15  style='cursor:pointer;' title='Edit record'  onclick=self.document.location='home.php?page=opd&OPDID=".$this->getValue("OPDID")."&action=Edit&PID=".$this->getOpdPatient()->getId()."'>";
		//$tools .= "<img   src='images/add_note.jpg' style='cursor:pointer;' title='Add Notes'  />";
		
		$out  = "<div class='".$this->css_Cont_class."' >\n";
			$out .= "<div class='opdBody' >\n";
			$out .= "<table border=0 width=100% cellspacing=0   class='opdTbl'>\n";
					$out .= "<tr>\n"; 
					$out .= "<td>".getPrompt("Type")." : </td><td>".$this->getValue("VisitType")." </td><td align=center>".getPrompt("Date and Time of visit")." : ".$this->getValue("DateTimeOfVisit")."</td><td>".getPrompt("Onset Date")." : ".$this->getValue("OnSetDate")."</td><td>".getPrompt("Doctor")." : ".$this->getOPDDoctor()."</td><td align='right'>".$tools."</td>\n";
					$out .= "</tr>\n";					
					$out .= "<tr>\n"; 
					$out .= "<td valign=top>".getPrompt("Complaints").": </td><td colspan=3>".$this->getValue("Complaint")."</td>\n";
					$out .= "</tr>\n";	
                                        if ($_SESSION["VISIT_ICD"]==1){
                                            $out .= "<tr>\n"; 
                                            $out .= "<td >ICD: </td><td colspan=3>".$icd->displayICD()."</td>\n";
                                            $out .= "</tr>\n";	
                                        }
                                        if ($_SESSION["VISIT_SNOMED"]==1){
                                            $out .= "<tr>\n"; 
                                            $out .= "<td >SNOMED : </td><td colspan=3>".$this->getValue("SNOMED_Text")."</td>\n"; //finding->getValue("TERM")
                                            $out .= "</tr>\n";
                                        }
					$out .= "<tr>\n"; 
					$out .= "<td >".getPrompt("Remarks").": </td><td>".$this->getValue("Remarks")."</td><td colspan=3 align=right>";
	...
	public function loadPrescriptionItems(){
		...
		$mdsDB->connect();		
		$sql="
		SELECT prescribe_items.PRS_ITEM_ID,drugs.Name,prescribe_items.Dosage,prescribe_items.HowLong,prescribe_items.Quantity,prescribe_items.Status,prescribe_items.Frequency FROM prescribe_items,drugs where prescribe_items.Active = 1 AND (prescribe_items.DRGID = drugs.DRGID OR prescribe_items.PDRGID = drugs.DRGID) AND prescribe_items.PRES_ID = '".$prs."' ";
		$result=$mdsDB->mysqlQuery($sql); 
