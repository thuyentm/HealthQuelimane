...
$stock =$_POST["Stock"];
	...
	for ( $i = 0 ; $i < count($txt); ++$i) {
		if ( $txt[$i] >= 1) {
			...
			$sql1="SELECT PDRGID,DRGID FROM prescribe_items  WHERE PRS_ITEM_ID = ".$txt[$i]." "; 
			...
			if ($result1) {
				...
				if ($row["DRGID"]==0) {
					$sql2="UPDATE drugs SET  ";
                                        ...
                                        $sql2.= " where DRGID = '".$row["PDRGID"]."' "; 
					$result2 = mysql_query($sql2);
					
					$sql3="UPDATE prescribe_items SET DRGID='$pdrgid' WHERE PRS_ITEM_ID = ".$txt[$i]." ";
