...
echo "<tr>";
echo "<td colspan=10 align=center class='t_head'><b>".$_SESSION["Hospital"]."<br>Hospital Performance Indicators  from ".$from_date->format("Y-m-d")." to ".$to_date->format("Y-m-d")."<br></b></td>";
echo "</tr>";
echo "<tr>";
...
while($wrow = $mdsDB->mysqlFetchArray($wres))  {	
	$sql_adm = " SELECT count(admission.ADMID)  from admission  WHERE  (admission.AdmitTo ='".$wrow["WID"]."' ) AND ( substring(admission.AdmissionDate,1,10) > '".$from."') AND (substring(admission.AdmissionDate,1,10) <= '".$to."')";
	$result_adm = $mdsDB->mysqlQuery($sql_adm);
    $row_adm = $mdsDB->mysqlFetchArray($result_adm);
	
	$sql_tr_in = " SELECT count(ADTR) from admission_transfer  WHERE  (TransferTo ='".$wrow["WID"]."' ) AND ( substring(TransferDate,1,10) > '".$from."') AND ( substring(TransferDate,1,10) <= '".$to."')";
	$res_tr_in = $mdsDB->mysqlQuery($sql_tr_in);
    $row_tr_in = $mdsDB->mysqlFetchArray($res_tr_in);

	$sql_dis_alive = " SELECT count(admission.ADMID)  from admission  WHERE  (admission.Ward ='".$wrow["WID"]."' )AND (admission.OutCome !='Died' ) AND ( substring(admission.DischargeDate,1,10) > '".$from."') AND (substring(admission.DischargeDate,1,10) <= '".$to."')";
	$res_dis_alive = $mdsDB->mysqlQuery($sql_dis_alive);
    $row_dis_alive = $mdsDB->mysqlFetchArray($res_dis_alive);
	
	$sql_dis_less_48 = " SELECT count(admission.ADMID) from admission  WHERE  (admission.Ward ='".$wrow["WID"]."' ) AND (admission.OutCome ='Died' ) and (DATEDIFF(substring(admission.DeathDate,1,10),substring(admission.AdmissionDate,1,10))<2 ) AND ( substring(admission.DischargeDate,1,10) > '".$from."') AND (substring(admission.DischargeDate,1,10) <= '".$to."')";
    $result_dis_less_48 = $mdsDB->mysqlQuery($sql_dis_less_48);
    $row_dis_less_48 = $mdsDB->mysqlFetchArray($result_dis_less_48);   
	
    $sql_dis_greater_48 = " SELECT count(admission.ADMID) from admission  WHERE  (admission.Ward ='".$wrow["WID"]."' ) AND (admission.OutCome ='Died' ) and (DATEDIFF(substring(admission.DeathDate,1,10),substring(admission.AdmissionDate,1,10))>=2 )AND ( substring(admission.DischargeDate,1,10) > '".$from."') AND (substring(admission.DischargeDate,1,10) <= '".$to."')";
    $result_dis_greater_48 = $mdsDB->mysqlQuery($sql_dis_greater_48);
    $row_dis_greater_48 = $mdsDB->mysqlFetchArray($result_dis_greater_48);   
	
	$sql_tr_out = " SELECT count(ADTR) from admission_transfer  WHERE  (TransferFrom ='".$wrow["WID"]."' ) AND ( substring(TransferDate,1,10) > '".$from."') AND ( substring(TransferDate,1,10) <= '".$to."')";
	$res_tr_out = $mdsDB->mysqlQuery($sql_tr_out);
    $row_tr_out = $mdsDB->mysqlFetchArray($res_tr_out);
