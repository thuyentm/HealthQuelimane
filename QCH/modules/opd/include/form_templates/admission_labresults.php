...
$query="select concat(p.Personal_Title,' ',p.Full_Name_Registered) as `name`,p.PID,p.DateOfBirth,p.Age,p.Personal_Civil_Status,p.Gender,p.NIC,concat(p.Address_Street,' ',p.Address_Village,' ',p.Address_DSDivision,' ',p.Address_District) as address,a.AdmissionDate,a.OnSetDate,a.Complaint,concat(u.Title,u.FirstName,' ',u.OtherName) as doctor,a.ICD_Text,a.SNOMED_Text,a.Remarks from patient as p,lab_order as lo,admission as a,`user` as u where lo.LAB_ORDER_ID=$lid and p.PID=lo.PID and a.ADMID=lo.OBJID and a.Doctor=u.UID";
...
$pdf->MultiCell(0,4,'','B');
$pdf->Ln();
$pdf->SetWidths(array(30,45,24,40));
