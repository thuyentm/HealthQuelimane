...
//                $Btns .= "<input type='button' value='Print this list' onclick=printTable('drugs_frequencyForm','Frequency');>";
			...
			case "mydrugsEdit":
				...
				$sql = "SELECT drugs.Name FROM Doctor_Drug, drugs where (Doctor_Drug.DRGID = drugs.DRGID ) AND (Doctor_Drug.DDID = $ddid)";
				$result1 = mysql_query($sql);
				while ($row= mysql_fetch_assoc($result1)){ 
				$drugname=$row['Name'];
				}			

                $out .= loadHeader($drugname);
                $out .= "<div id='prefCont' >" . loadForm("doctorDrugsFormEdit") . "</div>";
                break;	
