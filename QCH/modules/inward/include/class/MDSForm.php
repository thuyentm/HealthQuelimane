		...
		for ($i = 0; $i < count($this->FrmConf["FLD"]); ++$i)
		{
			...
			switch ($this->FrmConf["FLD"][$i]["Type"]) {
				...
				case "selectdrug":
					...
					break;
				case "selectdrugposition":
					$out .= $this->frmSelectDrugPosition($this->FrmConf,$i);
					break;	
				case "dosage":
 ...
 window.location='home.php?page=preferences&mod=VillageNew';
...
else
  {
	...
	private function frmSelectDrug($frm,$i){
		...
		private function frmSelectDrugPosition($frm,$i){
		$out = "";
		$out .= $this->fldName($frm["FLD"][$i]["Name"],$frm["FLD"][$i]["valid"]);
		if ($this->FrmObj) {
			$out .= $this->getPositionList($frm["FLD"][$i]["Id"],$this->FrmObj->getValue($frm["FLD"][$i]["Id"]));
		}	
		else {
			$out .= $this->getPositionList($frm["FLD"][$i]["Id"],$frm["FLD"][$i]["Value"]);
		}
		$out .= $this->fldHelp($frm["FLD"][$i]["Id"],$frm["FLD"][$i]["Help"]);
		return $out;
	}
...
EOT;
	...
	private function getPositionList($id,$value) {
		$mdsDB = MDSDataBase::GetInstance();
		$mdsDB->connect();		
 		$out = "";
		$sql="SELECT POSITIONID,Name ";
		$sql .= " FROM Drug_position WHERE  (Active = 1) " ;
		//AND (HID='".$_SESSION["HID"]."')
	...
	private function getFrequencyList($id,$value) {
		...
		while($row = $mdsDB->mysqlFetchArray($result))  {
			if ($row["WID"] == $value) {
				$out .="<option value='".$row["WID"]."' selected>".$row["Name"]."</option>\n";
			}
			else {
					$out .="<option value='".$row["WID"]."' >".$row["Name"]."</option>\n";
			}
		  }
		$out .="</select>\n";
		$mdsDB->close();
