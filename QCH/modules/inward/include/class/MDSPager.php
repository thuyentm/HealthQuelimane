...
if (alertText.match(/^.*Resusciation$/)) 
{ 
    $('#'+rowid).css({'background':'#1874CD'});
	
}

else if (alertText.match(/^.*Emergent$/)  || alertText.match(/^.*Pending$/)) 
{ 
   $('#'+rowid).css({'background':'#EE2C2C'});
}

else if (alertText.match(/^.*Less Urgent$/)) 
{ 
    $('#'+rowid).css({'background':'#9ACD32'});
}
