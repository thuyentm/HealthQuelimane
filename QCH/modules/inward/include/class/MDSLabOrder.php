...
class MDSLabOrder extends MDSPersistent
{
	...
	public function renderFormEdit($episode,$pat,$type){
		...
		$out .= "<tr><td colspan=4><hr style='border:0;color: #000000;background-color: #000000;height:1px;'></td></tr>";
			...
			if ($_GET["action"] != "View") {
				...
				//$out .= "<input id='rejectBtn' type='button'  class='BlnkBtn' value='Specimen Clotted (Reject)' onclick='saveDataReject()'>";
			}
	...
	private function getLabItems(){
		...
		$mdsDB1 = MDSDataBase::GetInstance();
		$mdsDB1->connect();		
		$sql1 = "SELECT lab_tests.Name,lab_order_items.LAB_ORDER_ITEM_ID,lab_order_items.LABID,lab_tests.DefValue,lab_tests.RefValue FROM lab_order_items,lab_tests WHERE (lab_order_items.LAB_ORDER_ID = '".$this->getId()."') AND (lab_order_items.LABID=lab_tests.LABID) Order by lab_tests.LABID";	
		$result1=$mdsDB1->mysqlQuery($sql1);
		...
		$i=0;
			...
			if ($row1["LAB_ORDER_ITEM_ID"]) {
				...
				$l_test->openId($row1["LABID"]);
				$out .= "<tr>";
				$out .= "<td valign=middle class='lab_td' >".$row1["Name"]."</td>";
				...
				$out .= " id ='".$l_item->getId()."' ";
					if ($l_item->getValue("TestValue")) {
						...
						//$out .= " value ='".$l_test->getValue("DefValue")."'" ;
					}
					if (!$l_item->getValue("TestValue")) {
						//$out .= " value ='".$l_item->getValue("TestValue")."'" ;	
						$out .= " value ='".$l_test->getValue("DefValue")."'" ;
